# QLM Zombie build42 deploy script (Posh-SSH)
# Usage: powershell -ExecutionPolicy Bypass -File deploy-build42.ps1
...
$HostPort = 4066
$HostUser = 'root'
$JarLocal = 'D:\mcmod\build\libs\qlmzombie-3.0.0.beta.build42.jar'
...
Write-Host " QLM Zombie build42 deploy (YSM enabled)" -ForegroundColor Cyan
...
try {
    ...
    Write-Host "[1/4] Uploading main jar (408.9 MB, ~20 min) ..." -ForegroundColor Cyan
    if ($keyFile) {
    ...
    Write-Host "[2/4] Uploading SeverAdmin scripts + quest config ..." -ForegroundColor Cyan
    ...
    if (Test-Path $QuestConfigLocal) {
        Write-Host "  Uploading FTB Quests config -> $QuestConfigRemote" -ForegroundColor Cyan
        # 逐文件上传 config/ftbquests/quests（Set-SCPItem 目录上传递归不可靠，逐文件最稳）
    ...
    Write-Host "Scripts + config uploaded" -ForegroundColor Green

    Write-Host "[3/4] Verifying remote jar ..." -ForegroundColor Cyan
    $verifyCmd = "cd $JarRemoteDir && python3 -m zipfile -t qlmzombie-3.0.0.beta.build42.jar > /dev/null 2>&1 && echo JAR_OK || echo JAR_BAD"
    $r = Invoke-SSHCommand -SessionId $session.SessionId -Command $verifyCmd -TimeOut 120
    ...
    Write-Host "[4/4] Running remote deploy.sh docker ..." -ForegroundColor Cyan
    $deployCmd = "cd $SeverAdminDir && nohup ./deploy.sh docker > /tmp/build42_deploy.log 2>&1 & echo DEPLOY_STARTED"
    ...
    Write-Host "Deploy log: tail -f /tmp/build42_deploy.log" -ForegroundColor Cyan
    Write-Host "========================================" -ForegroundColor Cyan
}
finally {
