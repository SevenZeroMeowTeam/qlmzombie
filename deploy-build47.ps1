# QLM Zombie build47 deploy script (Posh-SSH)
# Usage: powershell -ExecutionPolicy Bypass -File deploy-build47.ps1
...
$HostPort = 4066
$HostUser = 'root'
$JarLocal = 'D:\mcmod\build\libs\qlmzombie-3.0.0.beta.build47.jar'
...
Write-Host " QLM Zombie build47 deploy (atlas fix)" -ForegroundColor Cyan
...
try {
    ...
    $verifyCmd = "cd $JarRemoteDir && python3 -m zipfile -t qlmzombie-3.0.0.beta.build47.jar > /dev/null 2>&1 && echo JAR_OK || echo JAR_BAD"
    ...
    $deployCmd = "cd $SeverAdminDir && nohup ./deploy.sh docker > /tmp/build47_deploy.log 2>&1 & echo DEPLOY_STARTED"
    ...
    Write-Host "Deploy log: tail -f /tmp/build47_deploy.log" -ForegroundColor Cyan
    Write-Host "========================================" -ForegroundColor Cyan
}
finally {
