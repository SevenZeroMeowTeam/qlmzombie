# QLM Zombie build54 deploy script (Posh-SSH)
# Usage: powershell -ExecutionPolicy Bypass -File deploy-build54.ps1
...
$HostPort = 4066
$HostUser = 'root'
$JarLocal = 'D:\mcmod\build\libs\qlmzombie-3.0.0.beta.build54.jar'
...
Write-Host " QLM Zombie build54 deploy (building v7)" -ForegroundColor Cyan
...
try {
    ...
    $verifyCmd = "cd $JarRemoteDir && python3 -m zipfile -t qlmzombie-3.0.0.beta.build54.jar > /dev/null 2>&1 && echo JAR_OK || echo JAR_BAD"
    ...
    $deployCmd = "cd $SeverAdminDir && nohup ./deploy.sh docker > /tmp/build54_deploy.log 2>&1 & echo DEPLOY_STARTED"
    ...
    Write-Host "Deploy log: tail -f /tmp/build54_deploy.log" -ForegroundColor Cyan
    Write-Host "========================================" -ForegroundColor Cyan
}
finally {
