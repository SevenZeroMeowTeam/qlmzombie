...
PlayerEvents.loggedIn(event => {
    ...
    // KubeJS 6：使用 player.persistentData（player.stage 为旧版 API，已不存在 → 登录报错）
    if (!player.persistentData.getBoolean('qlm_welcomed')) {
        player.persistentData.putBoolean('qlm_welcomed', true);
