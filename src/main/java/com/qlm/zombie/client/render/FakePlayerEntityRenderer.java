    ...
    private void fetchSkinFromURLAsync(String url, FakePlayerEntity entity) {
        ...
        CompletableFuture.runAsync(() -> {
            try {
                ...
                connection.setRequestProperty("User-Agent", "Mozilla/5.0 (QLMZombieMod SkinLoader)");
                ...
                try (InputStream is = connection.getInputStream();
                    ...
                    if (imageBytes.length < 100) {
                        com.qlm.zombie.QLMZombieMod.LOGGER.warn("[SkinLoader] Failed to fetch skin from URL {}: too short ({})", url, imageBytes.length);
                        return;
                    }
                    ...
                    if (contentType != null && !contentType.toLowerCase().contains("image")
                            ...
                            && !(connection.getURL().getPath().contains("/preview/")
                                 || connection.getURL().getPath().contains("/skin/"))) {
                        com.qlm.zombie.QLMZombieMod.LOGGER.warn("[SkinLoader] Failed to fetch skin from URL {}: content type is not image ({})", url, contentType);
                        return;
                    }

                    final String base64URL = "data:image/png;base64," + Base64.getEncoder().encodeToString(imageBytes);

                    // 同步到主线程：直接缓存到 SKIN_CACHE，下次 getTextureLocation 就会命中
                    Minecraft.getInstance().execute(() -> {
                        try {
                            ResourceLocation skin = loadSkinFromBytes(imageBytes, entity);
                            SKIN_CACHE.put(url, skin);
                            // 同时把 base64 版本也缓存，防止 URL 再次请求
                            SKIN_CACHE.put(base64URL, skin);
                            com.qlm.zombie.QLMZombieMod.LOGGER.debug("[SkinLoader] Skin loaded for URL {} ({} KB)", url, imageBytes.length / 1024);
                        } catch (Exception ex) {
                            com.qlm.zombie.QLMZombieMod.LOGGER.warn("[SkinLoader] Failed to register skin texture: {}", ex.getMessage());
                        }
                    });
                }
            } catch (java.net.SocketTimeoutException e) {
                com.qlm.zombie.QLMZombieMod.LOGGER.warn("[SkinLoader] Timeout fetching skin {}: {}", url, e.getMessage());
            } catch (java.io.IOException e) {
                com.qlm.zombie.QLMZombieMod.LOGGER.warn("[SkinLoader] Failed to fetch skin {}: {}", url, e.getMessage());
            } catch (Exception e) {
                com.qlm.zombie.QLMZombieMod.LOGGER.warn("[SkinLoader] Unexpected error fetching skin {}: {}", url, e.getMessage());
            }
        });
    }
