...
public class FragmentGrenadeItem extends Item {

    public FragmentGrenadeItem() {
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        if (!player.level().isClientSide) {
            ThrownGrenadeEntity entity = new ThrownGrenadeEntity(
                    ...
                    player,
                    level,
                    ThrownGrenadeEntity.GrenadeType.FRAGMENT
            );
            entity.setPos(
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> tooltip, TooltipFlag flag) {
        tooltip.add(Component.literal("高爆破片手雷，爆炸产生致命碎片").withStyle(ChatFormatting.GRAY));
    }
}
