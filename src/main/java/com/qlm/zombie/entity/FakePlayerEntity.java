...
public class FakePlayerEntity extends PathfinderMob {
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(9, new BreakDoorGoal(this, (difficulty) -> true));
        this.goalSelector.addGoal(10, new AIEquipGoal(this));
        this.goalSelector.addGoal(11, new AIPickupLootGoal(this));
        this.goalSelector.addGoal(12, new AIWorkstationGoal(this));
    ...
    public InteractionResult mobInteract(Player player, InteractionHand hand) {
        ...
        if (!this.isTamed()) {
            if (stack.is(Items.BONE) || stack.is(Items.ROTTEN_FLESH) || stack.is(Items.COOKED_BEEF) ||
                    stack.is(Items.COOKED_PORKCHOP) || stack.is(Items.COOKED_CHICKEN) || stack.is(Items.COOKED_MUTTON) ||
                    stack.is(Items.COOKED_RABBIT) || stack.is(Items.COOKED_COD) || stack.is(Items.COOKED_SALMON) ||
                    stack.is(Items.BREAD) || stack.is(Items.APPLE) || stack.is(Items.GOLDEN_APPLE)) {
    ...
    private static class AIWorkstationGoal extends Goal {
        ...
        private String hasCraftingRecipe() {
            for (String[] recipe : CRAFTING_RECIPES) {
                if (hasMaterial(recipe[0])) return recipe[0] + "|" + recipe[1] + "|" + recipe[2];
            }
            return null;
        }
        ...
        private String hasSmeltingRecipe(WorkstationType type) {
            for (String[] recipe : SMELTING_RECIPES) {
                ...
                if (hasMaterial(recipe[0])) return recipe[0] + "|" + recipe[1] + "|1";
            }
            return null;
        }
