...
public class PlayerInitHandler {
    ...
    private static void sendModIntroduction(Player player) {
        ...
        player.displayClientMessage(Component.literal("  §c🔥 动态难度AI增强: §f僵尸/骷髅随天数增加速度/护甲/击退抗性/伤害/生命值"), false);
        player.displayClientMessage(Component.literal("  §f- 难度越高怪物AI越强: 更快追击、更高护甲、更强击退抗性、更多伤害"), false);
        ...
        player.displayClientMessage(Component.literal("  §f- 工作台100+配方: 工具/武器/护甲/建筑/红石/装饰等全支持"), false);
        ...
        player.displayClientMessage(Component.literal("  §b🤖 AI玩家聊天指令系统(Player2 MCP API):"), false);
        player.displayClientMessage(Component.literal("  §f- 聊天栏输入 [AI名字] + 指令 即可与AI玩家交互"), false);
        player.displayClientMessage(Component.literal("  §f- 支持: 跟随/等待/过来/收集/制作/给我/攻击/挖矿/砍树/守护/探索/治疗"), false);
        player.displayClientMessage(Component.literal("  §f- 示例: [AI名字] 跟我、[AI名字] 砍树、[AI名字] 给我10个木头"), false);
        player.displayClientMessage(Component.literal("  §f- 通过Player2 MCP API远程处理智能指令，支持自然语言理解"), false);
        player.displayClientMessage(Component.literal("  §a🆕 物品注册表: 200+中文物品名映射，AI可识别原版合成材料/配方/工作方块"), false);
        player.displayClientMessage(Component.literal("  §a🆕 任务自动跟随: 挖矿/砍树/收集/攻击等任务完成后自动切换跟随模式"), false);
        player.displayClientMessage(Component.literal("  §a🆕 执行任务时不跟随: 仅驯服的AI在任务期间不自动跟随，任务结束恢复跟随"), false);
        player.displayClientMessage(Component.literal("  §f- 可在 qlmzombie-common.toml 的 [player2_mcp] 中配置API地址和密钥"), false);
        ...
        player.displayClientMessage(Component.literal("§e⚔️ 武器/工具/盔甲合成品质系统:"), false);
        player.displayClientMessage(Component.literal("  §f- 每次合成随机生成品质与伤害，品质越高伤害越高(无上限)"), false);
        player.displayClientMessage(Component.literal("  §f- 7档品质: §8破损§f/§f普通§f/§a优良§f/§b稀有§f/§d史诗§f/§6传说§f/§c神话"), false);
        player.displayClientMessage(Component.literal("  §f- 神话级物品可破坏基岩、屏障，无视游戏规则限制(脱下后恢复)"), false);
