package com.qlm.zombie.thirst.content.registry;
        ...
        dispatcher.register(Commands.literal("thirst")
                ...
                .then(Commands.literal("query").then(Commands.argument("Player", EntityArgument.player())
                        .executes(context -> {
                                    ...
                                    if (iThirst == null) return 0;
                                    Object[] arg =new Object[2];
                                    arg[0]=iThirst.getThirst();
                                    arg[1]=iThirst.getQuenched();
                                    context.getSource().sendSuccess(()->MutableComponent.create(new TranslatableContents("command.qlmzombie.thirst.query","command.qlmzombie.thirst.query",arg)),false);
                                    return 0;
                                }
                        )))
                        ...
                        .then(Commands.argument("thirst", IntegerArgumentType.integer(0,20))
                                .then(Commands.argument("quenched", IntegerArgumentType.integer(0,20))
                                        .executes(context -> {
                                            ...
                                            context.getSource().sendSuccess(()->MutableComponent.create(new TranslatableContents("command.qlmzombie.thirst.set","command.qlmzombie.thirst.set",arg)),false);
                                            return 0;
                                        })))
                ))
                .then(Commands.literal("enable").then(Commands.argument("Player",EntityArgument.players())
                        .then(Commands.argument("bool", BoolArgumentType.bool())
                                .executes(context ->{
                                    ...
                                    if(shouldTick){
                                        context.getSource().sendSuccess(()->MutableComponent.create(new TranslatableContents("command.qlmzombie.thirst.enable","command.qlmzombie.thirst.enable",playersName.toArray())),false);
                                    }else {
                                        context.getSource().sendSuccess(()->MutableComponent.create(new TranslatableContents("command.qlmzombie.thirst.disable","command.qlmzombie.thirst.disable",playersName.toArray())),false);
                                    }
